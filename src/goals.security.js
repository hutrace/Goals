...
;(function() {
    ...
    var ajax_error = function(err) {
        if(Goals.prompt) {
            ...
            Goals.prompt.error('[' + err.code + '] ' + err.statusText);
        }
    }
    var ajax_success = function(res, okFn, failFn) {
        if(res.code === 0) {
            ...
            if(failFn) {
                ...
                Goals.prompt.hideLoading();
                Goals.prompt.warn('[' + res.code + '] ' + res.msg);
            }else {
                okFn && okFn(res);
            }
        }
    ...
    security.token = function(opt) {
        req.ajax({
            url: (common.SERVER_NAME + "/" + (opt.url || "/login/default")).replace("//", "/"),
            type: "post",
            data: opt.data,
    ...
    security.ajax = function(settings) {
        ...
        settings.url = (common.SERVER_NAME + "/" + (settings.url || "")).replace("//", "/");
        ...
        if(encrypt) {
            ...
            settings.data = security.aesEncode(user.secret, JSON.stringify(settings.data));
        }
        if(user) {
    ...
    req.setAjaxDataResolver("application/scpsat-json", function(text) {
        ...
        rsp.data = security.aesDecode(user.secret, rsp.data);
        try{
            rsp.data = JSON.parse(rsp.data);
        }catch(e){}
        return rsp;
    });
    security.download = function(settings, callback) {
        settings.success = function(res) {
            window.open(common.SERVER_NAME + '/system/download/' + res.data);
            callback && callback();
        }
        security.ajax(settings);
    }
    ...
    security.upload = function(settings) {
        settings.url = (common.SERVER_NAME + "/" + (settings.url || "")).replace("//", "/");
        /** @type {object} */
        var user = session.get(sessionTokenKey());
        if(user) {
        ...
        if(settings.data) {
            settings.url = settings.url + (settings.url.indexOf("?") > -1 ? "&" : "?") + object_2_qs(settings.data);
        }
        var xmlhr;
        ...
        if(settings.timeout) {
            xmlhr.timeout = settings.timeout;
            xmlhr.ontimeout = settings.error;
        }
        xmlhr.onerror = settings.error;
        xmlhr.onreadystatechange = function() {
            if(xmlhr.readyState === 4) {
                if(xmlhr.status === 200) {
                    
                }else {
                    settings.error && settings.error(xmlhr);
                }
            }
        }
        var form = new FormData();
        form.append('file', settings.file);
        xmlhr.open('POST', settings.url);
        xmlhr.send(form);
    }
    Goals.security = security;
}());
