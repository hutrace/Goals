;(function() {
    Goals.assert(Goals.prompt, "You must import the 'goals.prompt' package");
    Goals.assert(Goals.utils, "You must import the 'goals.utils' package");
    var ajax;
    ...
    function checkAndMerge(opt) {
        Goals.assert(opt.url, "You need to pass in the 'url' parameter");
        Goals.assert(opt.dataElem, "You need to pass in the 'dataElem' parameter");
        Goals.assert(opt.pageElem, "You need to pass in the 'pageElem' parameter");
    ...
    var GoalsPaging = function(settings) {
        ...
        Goals.utils.on(settings.pageElem, ".can-click", "click", function(e) {
            _this.run(e.getAttribute('page'));
        });
    };
    GoalsPaging.prototype.run = function(start) {
    ...
    GoalsPaging.prototype.template = function(list) {return '';};
    ...
    GoalsPaging.prototype.load = function(data) {
        ...
        if(allPaging > opt.pageNumber) {
            var _show_paging = parseInt((opt.pageNumber/2).toString());
